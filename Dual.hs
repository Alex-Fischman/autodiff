module Dual (Dual (..), evaluate, differentiate, derivative, gradient) where
...
derivative :: (Num a) => (Dual a -> Dual b) -> a -> b
derivative f x = differentiate . f $ Dual x 1

gradient :: Num a => ([Dual a] -> Dual b) -> [a] -> [b]
gradient f xs = map (differentiate . f . zipWith Dual xs) $ identity $ length xs
    where
        identity 1 = [[1]]
